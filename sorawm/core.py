    ...
    def run(
        ...
        for idx, frame in enumerate(
            ...
            if detection_result["detected"]:
                bbox_by_idx[idx] = detection_result["bbox"]
            else:
                bbox_by_idx[idx] = None
        ...
        for missed_idx in detect_missed:
            ...
            before_box = bbox_by_idx[before]
            after_box = bbox_by_idx[after]
            if before_box:
                bbox_by_idx[missed_idx] = before_box
            elif after_box:
                bbox_by_idx[missed_idx] = after_box

        # Second pass: stream frames again to avoid caching all frames in memory.
        input_video_loader = VideoLoader(input_video_path)
        for idx, frame in enumerate(
            tqdm(input_video_loader, total=total_frames, desc="Remove watermarks")
        ):
            bbox = bbox_by_idx.get(idx)
            if bbox is not None:
                x1, y1, x2, y2 = bbox
        ...
        # os.link(temp_output_path,output_video_path)
