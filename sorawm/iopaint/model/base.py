...
class InpaintModel:
    ...
    def _pad_forward(self, image, mask, config: InpaintRequest):
        ...
        if config.sd_keep_unmasked_area:
            # Keep arrays in float32 to avoid implicit float64 expansion on NumPy ops.
            mask_f = (mask.astype(np.float32) / 255.0)[:, :, np.newaxis]
            result_f = result.astype(np.float32)
            image_f = image[:, :, ::-1].astype(np.float32)
            result = (result_f * mask_f + image_f * (1.0 - mask_f)).astype(np.uint8)
        return result
